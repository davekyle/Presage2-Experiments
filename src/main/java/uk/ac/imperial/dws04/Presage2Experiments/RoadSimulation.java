...
//					state.setProperty("goals", goals.toString());
	...
	private RoadAgentGoals createNewAgentGoals() {
		return new RoadAgentGoals((Random.randomInt(maxSpeed)+1), Random.randomInt(3), Random.randomInt(10), Random.randomInt(3), Random.randomInt(2)+1);
	}
	...
	public void removeAgent(AgentLeftScenario e) {
		...
		this.sharedState.changeGlobal("participants", new StateTransformer() {
			...
			public Serializable transform(Serializable state) {
				...
				// hacky - only works because uuid = ipconhandle
				getIPConService().removeAgent(uuid);
				ids.remove(uuid);
				return ids;
			}
		});
